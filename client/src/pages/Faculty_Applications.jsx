...
export default function Faculty_Applications() {
  ...
  useEffect(() => {
    const fetchFacultyApplications = async () => {
      try {
        const response = await axios.get(`http://localhost:8000/api/leave/student`, {
          params: {
  ...
  const handleApproveFaculty = async (id, remarkFaculty) => {
    try {
      const response = await axios.patch(`http://localhost:8000/api/leave/student/approve/${id}`, { remarkFaculty });
  ...
  const handleDeclineFaculty = async (id, remarkFaculty) => {
    try {
      const response = await axios.patch(`http://localhost:8000/api/leave/student/decline/${id}`, { remarkFaculty });
      updateApplicationStatus(response.data, 'faculty');
    } catch (error) {
  ...
  const handleDiscussFaculty = async (id, remarkFaculty) => {
    try {
      const response = await axios.patch(`http://localhost:8000/api/leave/student/discuss/${id}`, { remarkFaculty });
      updateApplicationStatus(response.data, 'faculty');
  ...
  const handleApproveMentor = async (id, remarkMentor) => {
    try {
      const response = await axios.patch(`http://localhost:8000/api/leave/student/mentor/approve/${id}`, { remarkMentor });
  ...
  const handleDeclineMentor = async (id, remarkMentor) => {
    try {
      const response = await axios.patch(`http://localhost:8000/api/leave/student/mentor/decline/${id}`, { remarkMentor });
      updateApplicationStatus(response.data, 'mentor');
    } catch (error) {
      console.error("Error declining application:", error);
    }
  ...
  const handleDiscussMentor = async (id, remarkMentor) => {
    try {
      const response = await axios.patch(`http://localhost:8000/api/leave/student/mentor/discuss/${id}`, { remarkMentor });
      updateApplicationStatus(response.data, 'mentor');
    } catch (error) {
      console.error("Error discussing application:", error);
    }
  ...
  const viewApplication = async (id, role) => {
    try {
      const response = await axios.get(`http://localhost:8000/api/leave/student/${id}`);
      if (role === 'faculty') {
